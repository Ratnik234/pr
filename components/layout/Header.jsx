...
function AddButton() {
  ...
  return (
    <button
      id="header-add"
      onClick={() => setLogMealModalOpen(true)}
...
function Avatar() {
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-sm font-bold text-white leading-none select-none">{user?.name?.substring(0,2)?.toUpperCase() || 'U'}</span>
        <span
      ...
      <div className="hidden lg:block text-left">
        <p className="text-[13px] font-semibold leading-tight" style={{ color: 'var(--t-1)' }}>{user?.name || 'User'}</p>
